...
function ChatPageContent() {
      ...
      setConversations((previous) => {
        ...
        if (existing) {
          const updated = {
            ...existing,
            lastMessage: message,
            unreadCount: !isSelected && message.senderId === otherId ? existing.unreadCount + 1 : 0,
          };
  ...
  }, [recipientUser, selectedUserId, socket, t, user?.id]);
  ...
  }, [loadingMsgs, messages, selectedUserId]);
      ...
      <div className={`min-w-0 flex-1 flex-col bg-white ${!selectedUserId ? "hidden md:flex" : "flex"}`}>
        {selectedUserId && displayUser ? (
          <>
            ...
            <div className="hide-scrollbar flex-1 space-y-6 overflow-y-auto bg-gray-50/30 p-4 sm:p-6">
