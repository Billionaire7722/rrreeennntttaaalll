...
export default function ProfileHeader({
  ...
  const displayName = [user.lastName, user.firstName].filter(Boolean).join(" ") || user.name || "User";
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-white/60 bg-white shadow-[0_25px_80px_rgba(15,23,42,0.14)]">
      ...
      <div className="relative px-4 pb-5 pt-0 sm:px-6 sm:pb-6">
        <div className="flex flex-col gap-5 sm:-mt-12 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <button
              type="button"
              className="group relative -mt-10 h-24 w-24 overflow-hidden rounded-[1.75rem] border-4 border-white bg-gradient-to-br from-slate-900 via-teal-700 to-emerald-400 shadow-xl transition-transform hover:scale-[1.02] sm:-mt-14 sm:h-32 sm:w-32"
            ...
            <div className="min-w-0 space-y-3">
              ...
              <div className="min-w-0">
                <h1 className="text-2xl font-black leading-tight text-slate-900 sm:text-3xl lg:text-[2rem]">
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:min-w-[220px]">
            ...
            <p className="text-center text-xs leading-5 text-slate-500 sm:text-right">
              {t("change_limit") || "You can update your profile details here."}
            </p>
          </div>
        </div>
      </div>
